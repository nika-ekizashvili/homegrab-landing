document.addEventListener('DOMContentLoaded', () => {
  ...
  [closeTermsBtn, termsCloseFooterBtn].forEach(btn => {
    if (btn) btn.addEventListener('click', () => closeModal(termsModal));
  });
  ...
  simScrapeBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      extInactiveView.classList.add('hidden');
      extActiveView.classList.remove('hidden');

      // Show preview badge & update extension pane status
      dataPreviewBadge.classList.remove('hidden');
      ...
      paneSource.classList.remove('active-panel');
      paneExtension.classList.add('active-panel');
  ...
  const triggerInject = () => {
    ...
    setTimeout(() => {
      ...
      sheetPrice.textContent = `$${finalPrice.toLocaleString()}`;
      sheetArea.textContent = `${finalArea} m²`;

      sheetsRowPlaceholder.classList.add('hidden');
      sheetsRowData.classList.remove('hidden');

      // Swap active panel
      paneExtension.classList.remove('active-panel');
      paneTarget.classList.add('active-panel');
  ...
  simInjectBtn.addEventListener('click', triggerInject);
  if (simInjectMyHomeBtn) {
    simInjectMyHomeBtn.addEventListener('click', triggerInject);
  }
  ...
  simResetBtn.addEventListener('click', () => {
    ...
    dataPreviewBadge.classList.add('hidden');
    extInactiveView.classList.remove('hidden');
    extActiveView.classList.add('hidden');

    // Reset Inputs
    simSurchargeInput.value = '5';
    simRoundAreaChk.checked = true;
    ...
    simDescInput.value = 'ბინა ქირავდება ვაკეში, ჭავჭავაძის გამზირზე. ახალი რემონტით, ავეჯითა და ტექნიკით. აგენტი: Nika.';
    ...
    targetDesc.classList.add('desc-box');

    // Reset Sheets table
    sheetsRowPlaceholder.classList.remove('hidden');
    sheetsRowData.classList.add('hidden');

    // Reset active panels
    paneSource.classList.add('active-panel');
    paneExtension.classList.remove('active-panel');
    paneTarget.classList.remove('active-panel');
